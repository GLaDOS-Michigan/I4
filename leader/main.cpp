...
void init() {
    for (int i = 0; i < n; i++) {
        ...
        string distinct = "idn(N0) = ID0 & idn(N1) = ID1 & idn(N2) = ID2 & ";
...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        fprintf(stderr, "./main [model_name] [invariant_file]\n");
        return 0;
    }
    model_name = argv[1];

    read_invariants(fopen(argv[2], "r"));
    read_config(fopen(("config_" + model_name + ".txt").c_str(), "r"));

    check_input();

    init();
//    printf("vars[435]\n");
//    for (auto it : vars[435]) {
